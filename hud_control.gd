...
@onready var npcCompass = $BlueCompass
...
func _ready() -> void:
	...
	SystemEvents.update_npc_compass.connect(_update_npm_compass)
...
func _update_npm_compass(player: Node3D, dest_pos: Vector3):
	var direction = (dest_pos - player.global_transform.origin).normalized()
	var forward = -player.global_transform.basis.z.normalized()
	var angle = forward.angle_to(direction)
	var cross = forward.cross(direction).y
	...
	var tween = get_tree().create_tween()
	tween.tween_property(npcCompass, "rotation_degrees", -rad_to_deg(angle) - 90, 0.2)	
